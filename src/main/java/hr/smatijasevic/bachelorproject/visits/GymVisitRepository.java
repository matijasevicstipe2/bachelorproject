...
public interface GymVisitRepository extends JpaRepository<GymVisit, Long> {
    ...
    @Query("SELECT gv FROM GymVisit gv WHERE gv.account.id = :accountId AND gv.enterTime >= :startOfDay AND gv.enterTime < :startOfNextDay AND gv.exitTime IS NULL")
    List<GymVisit> findByAccountAndDate(@Param("accountId") Integer accountId,
